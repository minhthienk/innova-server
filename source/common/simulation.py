...
def put_data_pwm(cmdtype, addr, data):
    '''
    This static method is use for putting hex data into PWM templates 
    There are 2 cases:
    ...
    addr = str(addr)
    if len(addr)>2: addr = addr[1:]

    # determine byte count
    data_bytecount = len(data) # count the number of data bytes
    bytecount = data_bytecount 
    
    cmd = ''
    for i in range(1,int(bytecount/6)+1):
        cmd += PREFIX + header.format(addr, sid) + ' {} '*6  + POSTFIX # base cmd
        cmd += '\n'
    ...
    if (bytecount%6)!=0:
        ...
        if cmdtype=='res':
            ...
            cmd += PREFIX + header.format(addr, sid) + ' {} '*(bytecount%6)  + POSTFIX # base cmd
            cmd += '\n'

    if cmdtype=='res':
        data.extend(['00']*(6 - (bytecount%6)))
...
def put_data_iso(cmdtype, addr, data):
    '''
    This static method is use for putting hex data into PWM templates 
    There are 2 cases:
    + the number of hex data fits into 1 response
    ...
    for i in range(1,int(bytecount/6)+1):
        lenbyte = hex(10)[-1:] + '4'
        cmd += PREFIX + header.format(lenbyte, addr, sid) + ' {} '*6  + POSTFIX # base cmd
        cmd += '\n'

    if (bytecount%6)!=0:
        lenbyte = hex((bytecount%6) + 4)[-1:] + '4'
        cmd += PREFIX + header.format(lenbyte, addr, sid) + ' {} '*(bytecount%6)  + POSTFIX # base cmd
        cmd += '\n'
...
class FordSimulation:
    ...
    def __init__(self, ymme_dict):
        ...
        self.protocol = ymme_dict['protocol']
    ...
    def create_dtc(self):
        ...
        ymme_df = Database.filter_ymme(ymme_df, self.year , self.make, self.model, self.engine)
        ...
        for index, row in ymme_df.iterrows():
            ...
            # check protocol
            if protocol != self.protocol  : continue

            # TO-DO: will code later
            if protocol in ['PROTOCOL_PWM','PROTOCOL_ISO9141']: 

                res_add = str(profile_df.loc[profile, 'TagAddr/CanReq1'])
                ...
                for dtc in dtcs_sae:
                    ...
                    for definition in  dtc_def_list:
                        simdata += '//NOTE:  - ' + definition + '\n'


                if protocol=='PROTOCOL_PWM':
                    simdata += put_data_pwm(cmdtype='req', addr=req_add, data=['22','02','00']) + '\n'
                    simdata += put_data_pwm(cmdtype='res', addr=res_add, data=['62','02','00']) + '\n'

                    # request command
                    data = ['13', 'FF', 'FF']
                    simdata += put_data_pwm(cmdtype='req', addr=req_add, data=data) + '\n'

                    # response command
                    data = ['53']
                    data.extend(data_hex)
                    simdata += put_data_pwm(cmdtype='res', addr=res_add, data=data) + '\n'

                elif protocol=='PROTOCOL_ISO9141':
                    simdata += put_data_iso(cmdtype='req', addr=req_add, data=['22','02','00']) + '\n'
                    simdata += put_data_iso(cmdtype='res', addr=res_add, data=['62','02','00']) + '\n'

                    # request command
                    data = ['13']
                    simdata += put_data_iso(cmdtype='req', addr=req_add, data=data) + '\n'

                    # response command
                    data = ['53']
                    data.extend(data_hex)
                    simdata += put_data_iso(cmdtype='res', addr=res_add, data=data) + '\n'
